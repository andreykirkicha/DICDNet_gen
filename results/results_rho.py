...
mkdir('E:/DICDNet/results/metric_rho')
# mkdir('E:/DICDNet/results/LI_metric_rho')
...
for mask in MASKS:
    ...
    for metric in METRICS:
        ...
        # plt.title("mask " + str(mask) + " " + metric + " vs rho (LI)")
        plt.legend(labels=[f"phi = {phi}"])
        cur_path = 'E:/DICDNet/results/metric_rho/mask' + str(mask) + '/'
        # cur_path = 'E:/DICDNet/results/LI_metric_rho/mask' + str(mask) + '/'
        mkdir(cur_path)
        plt.savefig(cur_path + 'mask' + str(mask) + '_' + metric + '_rho.png', dpi=100)
        # plt.savefig(cur_path + 'LI_mask' + str(mask) + '_' + metric + '_rho.png', dpi=100)
        plt.clf()